const cats = [
  ...
  function generateKittenHTML(kitten) {
    const favouriteText = kitten.favourite ? " (любимчик)" : "";
    const kittenHTML = `
      <div class="kitten">
        <img src="${kitten.img_link}" alt="${kitten.name}">
        <h2>${kitten.name}${favouriteText}</h2>
        <p><b>Возраст:</b> ${kitten.age}</p>
        <p><b>Рейтинг:</b> ${kitten.rate}/10</p>
  ...
  function generateAllKittenHTML(kittens) {
    let allKittenHTML = "";
    kittens.forEach((kitten) => {
      const kittenHTML = generateKittenHTML(kitten);
      allKittenHTML += kittenHTML;
    });
    return allKittenHTML;
  }
  
  const kittenListContainer = document.getElementById("kitten-list");
  const allKittenHTML = generateAllKittenHTML(cats);
  kittenListContainer.innerHTML = allKittenHTML;
